...
import json
from sen_config import sensors0, sensors1


def load_config(version):
  if version:
    return sensors0 # Read and return the JSON file in nof folder
  else:
    return sensors1 # Read and return the JSON file in nof folder
...
def check_version():
  ...
  if not(p16()) and not(p17()) and p18():
    return True
  elif not(p16()) and  p17() and not(p18()):
    return True
  else:
    return False