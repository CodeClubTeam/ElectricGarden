...
class LED:
  ...
  def __init__(self, redpin = 'P12', greenpin ='P23'): 
    # Need to have options for both versions of design with change in LED polarity
    ...
    if not(p16()) and not(p17()) and p18():
      ...
      self.on = 1
      self.off = 0
    elif p15() and not(p16()) and p17() and not(p18()):
      self.version = '2.2'
      self.on = 1
      self.off = 0
    else:
      ...
      self.on = 0
      self.off = 1
    print('Hardware version: ', self.version)
