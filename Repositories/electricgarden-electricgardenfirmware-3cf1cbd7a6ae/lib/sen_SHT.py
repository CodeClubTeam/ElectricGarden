...
class SHT30():
  ...
  * https://www.sensirion.com/fileadmin/user_upload/customers/sensirion/Dokumente/11_Sample_Codes_Software/Humidity_Sensors/Sensirion_Humidity_Sensors_SHT3x_Sample_Code_V2.pdf
  ...
  def __init__(self, scl_pin='P11', sda_pin='P10', delta_temp = 0, delta_hum = 0, i2c_address=DEFAULT_I2C_ADDRESS):
    self.initialised=False

    try:
      self.i2c = I2C(2, mode=I2C.MASTER, pins=(sda_pin, scl_pin))
      self.initialised = True
    except:
      globals.MyLog.Record('Er_SenSHTBusFail',os.path.basename(__file__),'Failed to initialise SHT30 humidity sensor I2C bus' )
      self.initialised = False
      return
    ...
    try:    
      ...
      self.i2c.writeto(self.i2c_addr, SHT30.MEASURE_CMD) # Test if bus is responding
      time.sleep_ms(50)
      self.initialised = True
    except:
      globals.MyLog.Record('Er_SenSHTInitFail',os.path.basename(__file__),'Failed to initialise SHT30 humidity sensor' )
      self.initialised = False

    self.measure() # have added this in in case reading removes 0 values like everywhere else.
  ...
  def measure(self, raw=False):
    ...
    try:
      ...
      temp = float("%0.1f"%t_celsius) # Limit to one decimal place
      humi = float("%0.1f"%rh)
      ...
      globals.MyLog.Record('Er_SenSHTReadFail',os.path.basename(__file__),'Failed to read SHT30 humidity sensor' )
      return '', ''
