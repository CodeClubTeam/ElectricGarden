...
from pycom import wifi_on_boot, wdt_on_boot, heartbeat_on_boot, nvs_get, wdt_on_boot_timeout
...
def L01properties():
    ...
    u = uname()
    lora = LoRa(mode=LoRa.LORA, region=LoRa.AS923, power_mode=LoRa.SLEEP)
    lora_addr = lora.mac()
    return {
        'deepsleep': 'deepsleep',
        'ms_consts': 'ms_consts',
...
def G01properties():
    ...
    while not lte.isattached():
        ...
        if ctr > 20:
            ...
            ctr = ctr + 1
    if lte.isattached():
        iccid = lte.send_at_cmd("AT+SQNCCID?").split("\"")[1]
    else:
        iccid = 'error'
    return {
        'deepsleep': 'deepsleep',
