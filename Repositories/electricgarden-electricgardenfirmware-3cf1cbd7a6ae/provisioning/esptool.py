...
class ESPLoader(object):
    ...
    @staticmethod
    def detect_chip(port=DEFAULT_PORT, baud=ESP_ROM_BAUD, connect_mode='default_reset', tx_flasher=False):
        ...
        detect_port.connect(connect_mode, tx_flasher)
    ...
    def _connect_attempt(self, mode='default_reset', esp32r0_delay=False, flasher_on_tx=False):
        ...
        #
        # flasher_on_tx pulls tx low through a break command for alternate
        # boards that don't have DTR available and use tx to start flasher 
        last_error = None
        ...
        if mode != 'no_reset':
            #print("Opened port, standby, will use delay: {}, flasher tx: {}".format(esp32r0_delay, flasher_on_tx))
            #time.sleep(5)
            # Pull TX (break)) low to trigger flasher on alternate boards 
            if flasher_on_tx:
                print('B', end='')
                self._port.break_condition = True # Pull TX line LOW (Boot pin LOW)
                time.sleep(0.1)
            ...
            if flasher_on_tx:
                self._port.break_condition = False # Bring TX line HIGH (Boot pin HIGH)
                print('#', end='')
            time.sleep(0.1)
    ...
    def connect(self, mode='default_reset', tx_flasher=False):
        ...
        try:
            for _ in range(10):
                last_error = self._connect_attempt(mode=mode, esp32r0_delay=False, flasher_on_tx=tx_flasher)
                if last_error is None:
                    return
                last_error = self._connect_attempt(mode=mode, esp32r0_delay=True, flasher_on_tx=tx_flasher)
                if last_error is None:
                    return
        finally:
...
def exit_process(error_code, no_exit=False):
    if no_exit == True:
        print('Exit:', error_code)
    else:
        sys.exit(error_code)
...
def main():
    ...
    parser.add_argument('--no-exit', '-X',
                        help="Don't kill process during error, instead, print it as a Exit property",
                        dest='no_exit',
                        action='store_true',
                        default=False)
    ...
    parser.add_argument(
        '--tx-break',
        dest='txbreak',
        help='Should tx be pulled low to trigger the flasher',
        action='store_true',
        default=False)
    ...
    if args.operation is None:
        parser.print_help()
        return exit_process(1, args.no_exit)
    ...
    if operation_args[0] == 'esp':  # operation function takes an ESPLoader connection object
        ...
        if args.chip == 'auto':
            esp = ESPLoader.detect_chip(args.port, initial_baud, args.before, args.txbreak)
        else:
            chip_class = {
                'esp8266': ESP8266ROM,
                'esp32': ESP32ROM,
            ...
            esp.connect(args.before, args.txbreak)
...
def _main():
    try:
        ...
        exit_process(2, '-X' in sys.argv or '--no-exit' in sys.argv)
