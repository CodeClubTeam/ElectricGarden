...
def serial_to_batch(serial_string):
    # Note, whoever made this didn't test pycom's nvs capabilities and find that anything above 2^31 can't be saved properly. So anything bigger than type 2 is useless.
...
class ManufacturingRemote(object):
    ...
    def connect(self):
        if self.client is not None:
            raise Exception('connect() called when client is not None.')
        cosmos_client = MongoClient(CONNECTION_STRING)
        cosmos_collection = cosmos_client.db.manufacture
        self.client = cosmos_client 
        self.collection = cosmos_collection
    
    def insert_entity(self, type_arg, entity):
        if self.client is None:
            self.connect()
        entity['_type'] = type_arg
        inserted_entity = self.collection.insert_one(entity)
        return inserted_entity.inserted_id
    ...
    def reserve_serial(self, device_type_int, batch_number):
        ...
        latest_batch_document = self.collection.find_one_and_update(
            {
                '_type': 'batch',
                '_batch': batch_number
            },
            {
                '$inc': { 'units': 1, 'deviceCount.' + device_increment_key: 1 }, # Create a new 0 entry and then increment. Starts at 1
                '$set': { 'updated': timestamp() }
            },
            projection={
                'units': True # Only return units, I don't care about other fields, don't waste the internet traffic.
            },
    ...
    def _find_thing_query(self, query_properties):
        ...
        if '_id' in query_properties: # _id is the best way to find a thing
            query['_id'] = query_properties['_id']
        else:
            query['_type'] = 'thing'
            if '_hardware' in query_properties:
                query['_hardware'] = query_properties['_hardware']
            elif 'serial' in query_properties:
                query['serial'] = query_properties['serial']
            elif 'mac' in query_properties:
                query['mac'] = query_properties['mac']
            else:
    ...
    def get_entity_by_hardware(self, hardware_id, device_type):
        if not self.client:
            self.connect()
        
        # replace with find_thing
        thing_entity = self.collection.find_one(
            {
                '_type': 'thing',
        ...
        if thing_entity is None:
            # Replace with self.insert_entity
            ...
            # replace with find_thing
            thing_entity = self.collection.find_one({'_id': _id})
        print("Device _id: ", thing_entity['_id'])
        return thing_entity
    
    def update_thing(self, blob):
        if not self.client:
            self.connect()
        query = self._find_thing_query(blob)
        return self.collection.find_one_and_update(
            query,
            {
                '$set': blob
            },
            return_document=ReturnDocument.AFTER, # return final document
...
class Deployer(object):
    ...
    def pre_install(self):
        ...
        self._mpfe_exec("import uos\n")
        # self._mpfe_exec("pycom.bootmgr(fs_type=pycom.LittleFS, reset=False)\n")
        try:
            self._mpfe_exec('uos.fsformat("/flash")\n')
        except:
            self._mpfe_exec('os.mkfs("/flash")\n')
        self._mpfe_exec("pycom.nvs_erase_all()\n")
        self._mpfe_reset()
    ...
    def load_device(self):
        ...
        self.entity = self.remote.get_entity_by_hardware(hid, device_type)
    ...
    def is_L01node(self):
        ...
        return self.args.device == 'L01node'
    ...
    def device_properties(self):
        ...
        if self.args.device == 'L01node':
            self._mpfe_exec("import pycom")
            self._mpfe_exec("from machine import WDT")
            self._mpfe_exec("wdt = WDT(timeout=100000)")
            self._mpfe_exec("if hasattr(pycom, 'wdt_on_boot'): pycom.wdt_on_boot(False)\n")
            self._mpfe_exec("import nodesetup")
            props = self._mpfe_eval("nodesetup.L01properties()")
        else:
            self._mpfe_exec("import pycom")
            self._mpfe_exec("from machine import WDT")
            self._mpfe_exec("wdt = WDT(timeout=100000)")
            self._mpfe_exec("if hasattr(pycom, 'wdt_on_boot'): pycom.wdt_on_boot(False)\n")
            self._mpfe_exec("import nodesetup")
            props = self._mpfe_eval("nodesetup.G01properties()")
        return props

    def deploy(self):
        self.log.debug('Beginning deploy, args: %s', vars(self.args))
        self.pre_install()
        _is_L01node = self.is_L01node
        if ['G01node', 'L01node'][_is_L01node] != self.args.device:
            raise ValueError('This device looks like a %s, but you wanted to program a %s.' % (['G01node', 'L01node'][_is_L01node], self.args.device))
        self.load_device()       
        self.program(self.args.rom)        
        if self.args.copy:
            self.install(self.args.setup)
        else:
            self.install_img(self.args.setup)
        configuration = {
        ...
        if self.args.device == 'L01node':
            ...
            payload = {
               ...
               "connectivityPlanId": "iotnz-cs/iotnz-standard-cp"} # 'iotnz-cs/iotnz-standard-cp' if we want to connect
            try:
        ...
        if self.args.device == 'G01node':
            self._mpfe_exec("pycom.nvs_set('error_new', 'none')")
            self._mpfe_exec("pycom.nvs_set('error_flag', '0')")
            self._mpfe_exec("pycom.nvs_set('reset', '0')")
        self._mpfe_exec("pycom.nvs_set('count', 0)")
        self.log.info('Completed deploy of %s, serial: %s.', self.args.device, self.entity['serial'])
...
def main(arguments=None):
    ...
    parser.add_argument('-e', '--env', required=False, type=str, help='Env file to use for db access', default='dev')
    ...
    parser.add_argument('-r', '--rom', required=False, type=str, help='Firmware ROM Image to program', default=False) #default='artifacts/firmware.tgz
    parser.add_argument('-s', '--setup', required=False, type=str, help='Software FS Image to program ', default='artifacts/nodesetup.img')
    ...
    if not args.image:
        ...
        if args.device == 'L01node':
            args.image = 'artifacts/L01node_spiffs.img'
            if not args.rom:
                args.rom = 'artifacts/L01firmware.gz'
        elif args.device == 'G01node':
            args.image = 'artifacts/G01node_spiffs.img'
            if not args.rom:
                args.rom = 'artifacts/G01firmware.gz'
                #args.rom = 'artifacts/GPy-1.20.2.rc3.tar.gz'
        else:
            args.image = 'artifacts/gateway_spiffs.img'
            
    if args.env == 'dev':
        print('Provisioning to dev')
        load_dotenv('.env.development')
    elif args.env == 'prod':
        print('Provisioning to prod')
        load_dotenv('.env')
    
    global COSMOS_HOST
    COSMOS_HOST = os.getenv('COSMOS_HOST')
    global COSMOS_PORT
    COSMOS_PORT = int(os.getenv('COSMOS_PORT'))
    global COSMOS_COLLECTION
    COSMOS_COLLECTION = os.getenv('COSMOS_COLLECTION')
    global COSMOS_USERNAME
    COSMOS_USERNAME = os.getenv('COSMOS_USERNAME')
    global COSMOS_PASSWORD
    COSMOS_PASSWORD = os.getenv('COSMOS_PASSWORD')
    global COSMOS_STRING
    COSMOS_STRING = os.getenv('COSMOS_STRING')
