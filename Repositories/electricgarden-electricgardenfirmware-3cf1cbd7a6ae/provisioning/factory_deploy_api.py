...
class ManufacturingRemote(object):
    ...
    def get_entity_by_hardware(self, hardware_id, device_type):
        ...
        if thing_entity is None:
            _id = self.api.insert_thing({'_hardware': hardware_id, 'deviceType': device_type})
            print(_id)
            #thing_entity = self.api.find_thing({'_id': _id})
            thing_entity = self.api.get_thing_by_id(_id)
        return thing_entity
...
def main(arguments=None):
    ...
    if not args.image:
        ...
        if args.device == 'L01node':
            ...
            if not args.rom:
                args.rom = 'artifacts/L01firmware.tgz'
        elif args.device == 'G01node':
            args.image = 'artifacts/G01node_spiffs.img'
            if not args.rom:
                args.rom = 'artifacts/G01firmware.tgz'
        else:
