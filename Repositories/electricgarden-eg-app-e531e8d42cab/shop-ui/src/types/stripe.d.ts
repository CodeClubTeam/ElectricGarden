// NOTE: modified from https://github.com/streamich/stripe-web/blob/master/src/index.ts
...
export interface Stripe {
  ...
  confirmPaymentIntent(
    ...
    data: StripeConfirmPaymentIntentData,
  ...
  confirmPaymentIntent(
    clientSecret: string,
    data: StripeConfirmPaymentIntentData2,
  ): Promise<StripePaymentResult>;
  /**
  ...
  handleCardPayment(
    ...
    data?: StripeHandleCardPaymentData1,
  ): Promise<StripePaymentResult>;
  /**
  ...
  handleCardPayment(
    clientSecret: string,
    data?: StripeHandleCardPaymentData2,
  ): Promise<StripePaymentResult>;
  createToken();
  createSource();
  retrieveSource();
...
type StripeRedirectToCheckoutOptions =
  | {
      /**
...
export type StripePaymentResponse =
  | StripePaymentResponseToken
  | StripePaymentResponseSource;
...
export interface StripeElements {
  ...
  create(
    ...
    options?: StripeElementOptionsCard2,
  ): StripeElement;
  create(
    type: 'paymentRequest',
    options?: StripeElementOptionsPaymentRequest,
  ): StripeElement;
...
export interface StripeElement {
  ...
  on(
    event: 'blur' | 'change' | 'click' | 'focus' | 'ready',
    handler: (payload?: object) => void,
  );
  on(event: 'blur', handler: () => void);
  on(
    event: 'change',
    handler: (payload: StripeElementChangeEventPayload) => void,
  );
  on(
    event: 'click',
    handler: (payload: { preventDefault: () => void }) => void,
  );
  addEventListener(
    event: 'blur' | 'change' | 'click' | 'focus' | 'ready',
    handler: (payload?: object) => void,
