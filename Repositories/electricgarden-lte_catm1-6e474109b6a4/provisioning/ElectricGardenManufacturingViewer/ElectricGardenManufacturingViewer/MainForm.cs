...
namespace ElectricGardenManufacturingViewer
{
    public partial class MainForm : Form
    {
        ...
        private IQueries _queries = null;

        private static bool DynamicHasProperty(dynamic obj, string property)
        {
            return ((Newtonsoft.Json.Linq.JObject)obj).Property(property) != null;
        ...
        {
            devices = await _queries.GetDevices();
        ...
        private async Task UpdateLogs()
        {
            var index = listView1.SelectedIndices[0];
            var device = devices[index];
            var serial = device.serial.Value;
            List<LogDocument> logs = await _queries.GetLogs(serial.ToString());
        ...
        private async void printLabelToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                var device = devices[index];
                var serial = device.serial.Value;
                String type = device.deviceTypeName.Value == "node" ? "" : "GW:";
                if (device.deviceTypeName.Value == "gateway")
                {
            ...
            try
            {
                ...
                await Task.Run(async () =>
                {
                    ...
                    if (devices != null && devices.Count > 0)
                    {
                        ...
                        var orderedNodes = devices.Where(device => DynamicHasProperty(device, "deviceTypeName") && "node" == device.deviceTypeName.Value)
                        .OrderBy(device => device.batch.Value)
                        .ThenBy(device => device.batchUnit.Value)
                    .ToList();
                        var entries = await FriendlyNames.Entries();
                        for (var i = 0; i < orderedNodes.Count; i++)
                        {
                            var node = orderedNodes[i];
                            var serial = node.serial.Value;
                            int uid = i + 1;
