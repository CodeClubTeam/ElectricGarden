...
class FRAM_I2C(FRAM):
    ...
    def save_reading(self, reading):
        ...
        reading_address = 0x100 + (counter * 50)
    ...
    def grab_reading(self):
        ...
        reading_address = 0x100 + (int(counter) * 50)
        return self.read_address(reading_address, 50).decode("utf-8").strip('\x00')
    ...
    def reset(self):
        ...
        self.write(0x0, bytes([0]) * 20)
        self.write(0x0, str(0))
        self.write(0x10, str(0))
        self.write(0x20, str(1)) # To send data on restart then every second wake 

    def check_send(self):
        value = int(self.read_address(0x20, 1).decode("utf-8").strip('\x00'))
        if value == 1:
            self.write(0x20, str(0)) # Need to change to 0 to stop next send
            print("Routine set to send")
            return True
        else:
            self.write(0x20, str(1)) # Need to change to 1 to allow next send
