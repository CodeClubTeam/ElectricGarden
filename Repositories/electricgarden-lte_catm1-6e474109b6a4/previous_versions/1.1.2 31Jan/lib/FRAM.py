...
class FRAM_I2C(FRAM):
    ...
    def save_reading(self, reading, no_save):
        ...
        print(counter)
        self.write(0x10, bytes([0]) * 8) # Need to avoid reading old values ie 10 -> 9 but 0 still remains so get 90
        self.write(0x10, str(counter))
        reading_address = 0x100 + (counter * 75) # 0x0 to 0x100 is reserved for counter space
        max_address = 0x1FB4 # We can't write over the counter space (0x0 to 0x100) so can't start writing at max memory - size of packet
        min_address = 0x100 # Avoid writing over counter space
        act_address = reading_address % 0x2000 # FRAM loops over, so if reading from 0x2001, same as reading from 0x0001
        if act_address > max_address or act_address < min_address:
            print("Can't save over counters, increasing address value")
            self.write(0x10, bytes([0]) * 8) # Need to avoid reading old values ie 10 -> 9 but 0 still remains so get 90
            self.write(0x10, str(counter + 4)) # Counter already increased by one, but now increased by 4 to avoid counter space (0x0 -> 0x100 = 256 bytes. 75 * 4 = 300 bytes)
            reading_address = 0x100 + ((counter + 4) * 75)
    ...
    def grab_reading(self):
        ...
        if counter == '0' or counter == '':
            return False, 0
        if (int(counter) + 115) <= self.max_save(): # Max number of saves without overriding old data is 115. If value is 115 below max, no more data to resend
            print("Resent all saved readings")
            return False, 0
        print('Reading counter: {}'.format(counter))
        reading_address = 0x100 + (int(counter) * 75)
        max_address = 0x1FB4 # Opposite reasoning from save_reading, need to avoid reading random data from counter memory space
        min_address = 0x100
        act_address = reading_address % 0x2000
        if act_address > max_address or act_address < min_address:
            print("Can't read over counters, decreasing address value")
            reading_address = 0x100 + ((int(counter) - 4) * 75)
            value = 5 # Need to know how much to increase the counter back if the send is unsuccessful
        return self.read_address(reading_address, 75).decode("utf-8").strip('\x00'), value

    def max_save(self, value=0):
        max_save = int(self.read_address(0x28, 5).decode("utf-8").strip('\x00'))
        if value == 0:
            return max_save
        else:
            self.write(0x28, str(value))

    def increment(self):
        # To keep track of how many cycles we are up to.
        try:
    ...
    def dec_counter(self, value=1):
        # Reduces the reading counter by one on successful resend.
        counter = int(self.read_address(0x10, 5).decode("utf-8").strip('\x00'))
        if counter != 0:
            self.write(0x10, bytes([0]) * 8) # To avoid reading old values ie 10 -> 9 but 0 still remains so get 90
            self.write(0x10, str(counter - value))
            return False
        else:
    ...
    def reset(self):
        ...
        self.write(0x0, bytes([0]) * 256)
        self.write(0x0, str(0)) # Total deepsleep cycles
        self.write(0x8, str(0)) # Flag to request instructions from HQ
        self.write(0x10, str(0)) # Counter of unsent readings
        self.write(0x18, str(0)) # Flag to send bootup information
        self.write(0x20, str(0)) # Counter of USB powered sends
        self.write(0x25, str(1)) # To send data on restart then every second wake 
        self.write(0x28, str(0)) # Counter of the max missed readings counter
    ...
    def get_address(self):
        address = self.read_address(0x50, 125).decode("utf-8").strip('\x00')
        if address == '':
            return False
        else:
            return address
    
    def save_instructions(self, response):
        address = response.json()['settings']['readingsEndPoint']
        ### can save any other settings to nvram
        self.write(0x50, bytes([0]) * 127)
        self.write(0x50, address)

    def check_device_hq(self):
        if self.read_address(0x8, 1).decode("utf-8").strip('\x00') == '0':
            return True
        else:
            return False

    def check_bootup(self):
        if self.read_address(0x18, 1).decode("utf-8").strip('\x00') == '0':
            return True
        else:
            return False
