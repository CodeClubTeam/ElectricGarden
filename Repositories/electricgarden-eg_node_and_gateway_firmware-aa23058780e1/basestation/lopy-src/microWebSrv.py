...
class MicroWebSrv:
    ...
    def Start(self, threaded=True):
        ...
        if not self._started:
            ...
            if threaded:
                return MicroWebSrv._tryStartThread(self._serverProcess)
            else:
                self._serverProcess()
    ...
    def GetMimeTypeFromFilename(self, filename, disregardGz=False):
        filename = filename.lower()
        if disregardGz and filename[-3:] == '.gz':
            filename = filename[:-3]
    ...
    def Alias(self, url, target):
        self._aliases[url] = target
    ...
    def _physPathFromURLPath(self, urlPath):
        if urlPath in self._aliases:
            # Mind your head for Recursive invocation
            return self._physPathFromURLPath(self._aliases[urlPath])
        if urlPath == '/':
            for idxPage in self._indexPages:
                physPath = self._webPath + '/' + idxPage
                physPathGz = physPath + '.gz'
                if MicroWebSrv._fileExists(physPath):
                    return physPath
                if MicroWebSrv._fileExists(physPathGz):
                    return physPathGz
        else:
    ...
    class _client:
        ...
        def _processStaticFile(self, response, filepath, contentType):
            header = {}
        ...
        def _processRequest(self):
            ...
            try:
                    ...
                    if self._parseHeader(response):
                        ...
                        if not upg:
                            ...
                            if routeHandler:
                                ...
                                if filepath:
                                    contentType = self._microWebSrv.GetMimeTypeFromFilename(filepath, True)
                                    self._processStaticFile(response, filepath, contentType)
                                else:
                ...
                print('[Web] Internal Server Error')
                print(iex)
        ...
        def _parseFirstLine(self, response):
            try:
                ...
                if len(elements) == 3:
                    ...
                    if len(elements) > 0:
                        self._resPath = MicroWebSrv._unquote_decode(
                            elements[0])
                        if len(elements) > 1:
                            ...
                            for s in elements:
                                ...
                                if len(param) > 0:
                                    ...
                                    self._queryParams[MicroWebSrv._unquote_decode(
                                        param[0])] = value
                    return True
            except:
                pass
    ...
    class _response:
        ...
        def WriteResponseRedirect(self, location):
            ...
            return self.WriteResponse(303, headers, None, None, None)
