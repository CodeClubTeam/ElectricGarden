...
COSMOS_HOST = 'electric-garden-cosmos.documents.azure.com'
COSMOS_PORT = 10255
COSMOS_COLLECTION = 'manufacture'
COSMOS_USERNAME = 'electric-garden-cosmos'
COSMOS_PASSWORD = 'pbioYYhHUktiS0I28N6CnOKw8GMXvAPlLDjtCdDFuIOM1k1yiHTC1OvxWkh5ZiCojFSET21LdaoiR4Tf8onAzg==' # Secondary 
...
class ManufacturingRemote(object):
    ...
    def connect(self):
        ...
        cosmos_client = MongoClient(COSMOS_HOST, COSMOS_PORT, ssl=True, username=COSMOS_USERNAME, password=COSMOS_PASSWORD)
        cosmos_collection = cosmos_client.admin.manufacture
    ...
    def find_rom_images(self, romtar, force=False):
        ...
        for member in tar.getmembers():
            ...
            if filename == 'application.bin':
                ...
                bootloader = member
        if not application:
            self.log.debug('application.bin is missing from ROM')
        if not partitions:
...
class Deployer(object):
    ...
    def device_properties(self):
        ...
        self._mpfe_exec("from setup import properties")
        props = self._mpfe_eval("properties()")
        return props
